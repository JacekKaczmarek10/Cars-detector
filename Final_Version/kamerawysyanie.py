...
#rysowanie parkinów na pierwszym obrazie
            #p11=int(p11)
            #p12=int(p12)
            #p22=int(p22)
            #p21=int(p21)
...
# funkcjia wyszukująca krawędzie w obrazie rużnic 
...
def W_F(x1,y1,x2,y2):
    #przybliżenie funkcji x=b do funkcji bardzo szybko rosnącej 
    if x1==x2:
        a=r2
        b=x1
        return a,b
...
#@wysyłąnie zdiecia pustego parkingu
...
#Jeśli zaznaczamy miejsca parkingowe sami stawiamy l==0
    if l==5:
        l=1
        f=1
...
#wyświetlanie informacji
    ...
    cv.imshow("src2", src2)
#wysyłanie aktualnego zdjęcia parkingu
    w_plik='2.jpg'
    ...
    params = {'name': name}
    requests.post(url, files=files,params=params)
#wysyłanie aktualnego stanu parkingu
